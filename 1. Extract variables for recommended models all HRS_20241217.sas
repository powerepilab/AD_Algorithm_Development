...
libname rand 'F:\power\HRS\RAND_HRS\sasdata\2020(V2)';
options fmtsearch = (rand.formats);
libname raw 'F:\power\HRS\HRS data (raw)\SAS datasets';


data sasfmts; set rand.sasfmts; run;
proc format library=rand cntlin=sasfmts; run;

/**********************************************************************************************************************
*
*	Created 2019_0308, Updated 2019_0508, 2019_1202, 2020_0804, 2021_1109, 2022_0519, 2024_1217 
*		--> 2019_0528 - cleaned up code for upload to GitHub (data through 2014)
*		--> 2020_0804 - added wave-specific stratum and secu variables - raehsamp and raestrat
*			(they change from pre/post 2016) to extend to 2016		
*		--> 2021_1109 uses 2018(V1) RAND data to produce output through 2016
*		--> 2022_0519 updated to use interim cognitive and weight data (interim only, not released)
*				--> 2018 should be considered interim - not using updated RAND, but pulling in imputed cognition 
*					from the separate cross-wave files.
*		--> 2024_1217 updated to use 2020(V2) RAND data and to produce output through 2020, omits web respondents
*
*
*
*		- Extracts Variables used in Hurd model, Expert Model, and LASSO Model for application to 1998-2016+ HRS waves
...
*
*		- Note: Starting in 2018, HRS started administering some interviews via WEB.  This changed the cognitive battery, and thus
*				not all cognitive data needed for the algorithms is available for web respondents.  HRS does not currently impute 
*				these missing mesures (i.e. president, VP, cactus, scissor naming).  Thus, we do not provide dementia probabilities or classifications
...
data base; set rand.randhrs1992_2020v2 (keep = hhid pn raestrat raehsamp /*cluster & strata variables*/
									 hacohort ragender 
...
*NOTE: uses merged data, not released RAND 2018 V1 data
********************************************************************************************************************************************/
*wave 94;
%macro ext (w, y);
/*extract*/
	data wave_&y; 
		set rand.randhrs1992_2020v2 
		(keep = hhid pn 
...
*waves 96-16 - extract age and self-cognition;
%macro ext (w, y);
/*extract*/
	data wave_&y; 
	set rand.randhrs1992_2020v2
...
%ext(10, 10) %ext(11, 12) %ext(12, 14) %ext(13, 16) 


*waves 18-20 - extract age and self-cognition - TEL/INP respondents only;
%macro ext (w, y);
/*extract*/
	data wave_&y; 
	set rand.randhrs1992_2020v2
	(keep = hhid pn 
		...
		r&w.proxy /*indicator for proxy interview*/
		/*Cognition for self-respondents: TICS items*/
		r&w.mop r&w.dyp r&w.yrp r&w.dwp /* dates - for each one: 0=Incorrect, 1=Correct  */
		r&w.bwc20p /* TICS serial backwards count 0=Incorrect, 1=CorrectTry2, 2=CorretTry1 */
		r&w.ser7p /* TICS serial 7's 0-5 */
		r&w.cactp /*TICS object naming (cactus) - 0=Incorrect, 1=Corect*/
		r&w.scisp /*TICS object naming (scissors) - 0=Incorrect, 1=Corect*/
		r&w.presp r&w.vpp /*Wu: TICS president and VP naming - 0=Incorrect, 1=Correct*/
		r&w.imrcp /* immediate word recall - 0-10 */
		r&w.dlrcp); /* delayed word recall 0-10 */
		...
		*immediate and delayed word recall;
		if 0 le r&w.imrcp le 10 then iword_&y = r&w.imrcp;
			label iword_&y = "immediate word recall: 0-10, wave &y";
		if 0 le r&w.dlrcp le 10 then dword_&y = r&w.dlrcp;
			label dword_&y = "delayed word recall: 0-10, wave &y";
		iwordsq_&y = iword_&y * iword_&y; 
			label iwordsq_&y = "iword_squared: 0-100, wave &y";

		/*TICS variables - asked of everyone*/
		if 0 le r&w.ser7p le 5 then ser7_&y = r&w.ser7p;
			label ser7_&y = "TICS serial 7 score: 0-5, wave &y";
		*backwards count, scale 0-2 (for LASSO);
		if 0 le r&w.bwc20p le 2 then bwc_&y = r&w.bwc20p;
			label bwc_&y = "TICS backwards count score: 0-2, wave &y";
		*backwards count, scale 0-1 (for Hurd);
		if bwc_&y = 2 then bwc1_&y = 1; else if bwc_&y in (0, 1) then bwc1_&y = 0; 
		label bwc1_&y = "BackwardsCount: 1=Correct 1st attempt ONLY, 0=Incorrect or Correct 2nd attempt, wave &y";

		drop r&w.imrcp r&w.dlrcp r&w.proxy r&w.ser7p r&w.bwc20p; 
	run;
	...
	%macro tics(v, var);
		...
		run;
	%mend;
	%tics(mop, ticsmo) %tics(dyp, ticsdt) %tics(yrp, ticsyr) %tics(dwp, ticswk) 
	%tics(presp, pres) %tics(vpp, vp) %tics(cactp, cact) %tics(scisp, scis)
...
/***************************************************************************************
	Need ADLs and IADLs in 94/96 for purposes of computing change
*****************************************************************************************/
*waves 94-96;
%macro ext(w, y);
data wave_&y;
set rand.randhrs1992_2020v2
	(keep = hhid pn 
			r&w.iadl5a r&w.adl5a); /*IADLs: phone, money, meds, shop, meal*/
		
		if 0 le r&w.iadl5a le 5 then iadl_&y = r&w.iadl5a;
			label iadl_&y = "IADLs: phone, money, meds, shop, meal, wave &y";
		if 0 le r&w.adl5a le 5 then adl_&y = r&w.adl5a;
			label adl_&y = "ADLs: bath, eat, dress, walk across room, bed, wave &y";

drop r&w.iadl5a r&w.adl5a;
...
data base;
	merge base wave_&y (keep = hhid pn iadl_&y adl_&y);
	by hhid pn;
...
%macro ext(w, y);
	data wave_&y;
		...
		(keep = hhid pn ragender
				...
				r&w.adl5a  /*ADLs |
-- not available in RAND 2020 V2  
				r&w.adlc  change in ADLs -- not available in RAND 2020 V2  
				r&w.adlf /*flag for wave missed for computing change (i.e. interview waves skipped) 
				*/
				r&w.iadl5a /*IADLs: phone, money, meds, shop, meal*/
				r&w.eata r&w.batha r&w.dressa r&w.phonea
				r&w.shlt r&w.shltc r&w.shltcf /*self-reported health: 1=excellent, 5=poor | (RAND computed) change in self-reported health: positive = deteriorate
				flag for wave missed for computing change (i.e. interview waves skipped)*/
		...
		*adls, iadls;
		...
		if r&w.iadl5a = 0 then iadl_d_&y = 0; 
		else if r&w.iadl5a ge 1 then iadl_d_&y = 1;
			label iadl_d_&y = "has at least one IADL limitation, wave &y";

		if  r&w.adl5a = 0 then adl_d_&y = 0; 
		else if  r&w.adl5a ge 1 then adl_d_&y = 1;
		...
		*change flag;
		if r&w.shltcf not in (., .M) then randchf_&y = r&w.shltcf;
			label randchf_&y = "(from RAND) number waves skipped for computing change, wave &y";

		*create standardized change health status ;
		...
		*sampling weight;
		...
		drop ragender r&w.adl5a r&w.iadl5a r&w.eata r&w.batha r&w.dressa r&w.phonea
...
%mend;
												   ...
												   %ext(4, 98)
%ext( 5, 00) %ext(6, 02) %ext(7, 04)  %ext(8, 06)  %ext(9, 08)
%ext(10, 10) %ext(11,12) %ext(12, 14) %ext(13, 16) %ext(14, 18)
%ext(15, 20)
...
*2016;
%jorm (PD501,
	   PD506 PD509 PD512 PD515 PD518 PD521 PD524 PD527 PD530 PD533 PD536 PD539 PD542 PD545 PD548 PD551, 
	   PD507 PD510 PD513 PD516 PD519 PD522 PD525 PD528 PD531 PD534 PD537 PD540 PD543 PD546 PD549 PD552,
       PD508 PD511 PD514 PD517 PD520 PD523 PD526 PD529 PD532 PD535 PD538 PD541 PD544 PD547 PD550 PD553,
	   PD506, 16, h16d_r);
*2018;
%jorm (QD501,
	   QD506 QD509 QD512 QD515 QD518 QD521 QD524 QD527 QD530 QD533 QD536 QD539 QD542 QD545 QD548 QD551, 
	   QD507 QD510 QD513 QD516 QD519 QD522 QD525 QD528 QD531 QD534 QD537 QD540 QD543 QD546 QD549 QD552,
       QD508 QD511 QD514 QD517 QD520 QD523 QD526 QD529 QD532 QD535 QD538 QD541 QD544 QD547 QD550 QD553,
	   QD506, 18, h18d_r);

*2020;
%jorm (RD501,
	   RD506 RD509 RD512 RD515 RD518 RD521 RD524 RD527 RD530 RD533 RD536 RD539 RD542 RD545 RD548 RD551, 
	   RD507 RD510 RD513 RD516 RD519 RD522 RD525 RD528 RD531 RD534 RD537 RD540 RD543 RD546 RD549 RD552,
       RD508 RD511 RD514 RD517 RD520 RD523 RD526 RD529 RD532 RD535 RD538 RD541 RD544 RD547 RD550 RD553,
	   RD506, 20, h20d_r);
...
%cjorm(16, h16d_r, PD554, PD555, PD556, PD557, PD501); 
%cjorm(18, h18d_r, QD554, QD555, QD556, QD557, QD501); 
%cjorm(20, h20d_r, RD554, RD555, RD556, RD557, RD501); 
...
%ext(16, h16g_r, PG086, PG195, PG196, PG198, PG199, PG200)
%ext(18, h18g_r, QG086, QG195, QG196, QG198, QG199, QG200)
%ext(20, h20g_r, RG086, RG195, RG196, RG198, RG199, RG200)
...
%soc(08) %soc(10) %soc(12) %soc(14) %soc(16) %soc(18) %soc(20)
...
%macro ch(v);
	...
	/*2008 (w9)*/
 	...
 	/*2014 (w12)*/
		...
		else if &v._94 NE . then &v.ch_16 = &v._16 - &v._94;
		label &v.ch_16 = "change in &v between last non-missing interview and 2016 (non-standardized, no lookback limit)";

	 	/*2018 (w14)*/
		if &v._16 NE . then &v.ch_18 = &v._18 - &v._16; 
		else if &v._14 NE . then &v.ch_18 = &v._18 - &v._14; 
		else if &v._12 NE . then &v.ch_18 = &v._18 - &v._12; 
		else if &v._10 NE . then &v.ch_18 = &v._18 - &v._10; 
		else if &v._08 NE . then &v.ch_18 = &v._18 - &v._08; 
		else if &v._06 NE . then &v.ch_18 = &v._18 - &v._06; 
		else if &v._04 NE . then &v.ch_18 = &v._18 - &v._04; 
		else if &v._02 NE . then &v.ch_18 = &v._18 - &v._02; 
		else if &v._00 NE . then &v.ch_18 = &v._18 - &v._00; 
		else if &v._98 NE . then &v.ch_18 = &v._18 - &v._98; 
		else if &v._96 NE . then &v.ch_18 = &v._18 - &v._96; 
		else if &v._94 NE . then &v.ch_18 = &v._18 - &v._94;
		label &v.ch_18 = "change in &v between last non-missing interview and 2018 (non-standardized, no lookback limit)";

		/*2020 (w15)*/
		if &v._18 NE . then &v.ch_20 = &v._20 - &v._18; 
		else if &v._16 NE . then &v.ch_20 = &v._20 - &v._16; 
		else if &v._14 NE . then &v.ch_20 = &v._20 - &v._14; 
		else if &v._12 NE . then &v.ch_20 = &v._20 - &v._12; 
		else if &v._10 NE . then &v.ch_20 = &v._20 - &v._10; 
		else if &v._08 NE . then &v.ch_20 = &v._20 - &v._08; 
		else if &v._06 NE . then &v.ch_20 = &v._20 - &v._06; 
		else if &v._04 NE . then &v.ch_20 = &v._20 - &v._04; 
		else if &v._02 NE . then &v.ch_20 = &v._20 - &v._02; 
		else if &v._00 NE . then &v.ch_20 = &v._20 - &v._00; 
		else if &v._98 NE . then &v.ch_20 = &v._20 - &v._98; 
		else if &v._96 NE . then &v.ch_20 = &v._20 - &v._96; 
		else if &v._94 NE . then &v.ch_20 = &v._20 - &v._94;
		label &v.ch_20 = "change in &v between last non-missing interview and 2020 (non-standardized, no lookback limit)";


	proc means nolabels; var &v.ch_98 &v.ch_00 &v.ch_02 &v.ch_04 &v.ch_06 &v.ch_08 &v.ch_10 &v.ch_12 &v.ch_14 &v.ch_16 &v.ch_18 &v.ch_20;
	run;
%mend;
%ch(adl)
%ch(iadl) 
...
/**look at raw means of iword and dword - matches documentation;*/
/*proc means data=base;*/
/*var iword_14 iword_16 iword_18 iword_20 dword_14 dword_16 dword_18 dword_20; run;*/
...
%macro ch(v);
	...
	data base; set base;
		...
		*2016 (w13);
		if randchf_16 = 0 then &v._sdch2_16 = &v._16 - &v._14;
		else if randchf_16 = 1 then &v._sdch2_16 = (&v._16 - &v._12)/2;
		else if randchf_16 = 2 then &v._sdch2_16 = (&v._16 - &v._10)/3;
		*2018 (w14);
		if randchf_18 = 0 then &v._sdch2_18 = &v._18 - &v._16;
		else if randchf_18 = 1 then &v._sdch2_18 = (&v._18 - &v._14)/2;
		else if randchf_18 = 2 then &v._sdch2_18 = (&v._18 - &v._12)/3;
		*2020(w15);
		if randchf_20 = 0 then &v._sdch2_20 = &v._20 - &v._18;
		else if randchf_20 = 1 then &v._sdch2_20 = (&v._20 - &v._16)/2;
		else if randchf_20 = 2 then &v._sdch2_20 = (&v._20 - &v._14)/3;

		proc means nolabels; var &v._98 &v._sdch2_98  &v._04 &v._sdch2_04  &v._08 &v._sdch2_08 &v._10 &v._sdch2_10 &v._14 &v._sdch2_14 &v._16 &v._sdch2_16
			&v._18 &v._sdch2_18  &v._20 &v._sdch2_20;
	run;
%mend;
...
/**check to see if change in ADLs are close to RAND2018V1 versions - confirmed mean, SD, min, max are close;*/
/*proc means data=base nolabels;*/
/*var adlch_00--adlch_18; run;  */
...
run;
%mend;
%ch(20, 18) %ch(18, 16) %ch(16, 14) %ch(14, 12) %ch(12, 10) %ch(10, 08) %ch(08, 06) %ch(06, 04) %ch(04, 02) %ch(02, 00)
%ch(00, 98) %ch(98, 96)
...
proc means data=base;
var proxy_12 proxy_14 proxy_16 proxy_18 IQCODE_12 IQCODE_14 IQCODE_16 IQCODE_18 IQCODE_20;
run;

%macro imp(var);
data base; set base;
	array prior[*] &var._94 &var._96 &var._98 &var._00 &var._02 &var._04 &var._06 &var._08 &var._10 &var._12 &var._14 &var._16 &var._18;
	array var[*] &var._96 &var._98 &var._00 &var._02 &var._04 &var._06 &var._08 &var._10 &var._12 &var._14 &var._16 &var._18 &var._20;
	array imp[*] &var._i_96 &var._i_98 &var._i_00 &var._i_02 &var._i_04 &var._i_06 &var._i_08 &var._i_10 &var._i_12 &var._i_14 &var._i_16 &var._i_18 &var._i_20;
	array flag[*] &var._if_96 &var._if_98 &var._if_00 &var._if_02 &var._if_04 &var._if_06 &var._if_08 &var._if_10 &var._if_12 &var._if_14 &var._if_16 &var._if_18 &var._if_20;
	array proxy[*] proxy_96 proxy_98 proxy_00 proxy_02 proxy_04 proxy_06 proxy_08 proxy_10 proxy_12 proxy_14 proxy_16 proxy_18 proxy_20;
	...
	*for 1994 wave (where there is no prior wave), set imputed version equal to non-imputed version for all;
	...
	proc means nolabels;
		var &var._94 &var._i_94 &var._96 &var._i_96 &var._if_96 &var._98 &var._i_98 &var._if_98 &var._00 &var._i_00 &var._if_00 &var._02 &var._i_02 &var._if_02 
			...
			&var._12 &var._i_12 &var._if_12 &var._14 &var._i_14 &var._if_14 &var._16 &var._i_16 &var._if_16 &var._18 &var._i_18 &var._if_18 
			&var._20 &var._i_20 &var._if_20 ;
run;
%mend;
...
/*var hhid pn proxy_14 IQCODEch_14 IQCODEch_12 IQCODEch_i_14 IQCODEch_if_14 ; run;*/
/**/
/*proc means data=base; var proxy_14 IQCODEch_14 IQCODEch_12 IQCODEch_i_14 IQCODEch_if_14; run;*/
/*data test; set base;*/
/*if IQCODEch_14 ne . then IQCODEch_14_present=1; else IQCODEch_14_present=0;*/
/*if IQCODEch_12 ne . then IQCODEch_12_present=1; else IQCODEch_12_present=0;*/
/*run;*/
/**/
/*proc freq data=test;*/
/*tables proxy_14*IQCODEch_14_present*IQCODEch_12_present/list missing;*/
/*run;*/
...
run;
%mend;
%lab(96) %lab(98) %lab(00) %lab(02) %lab(04) %lab(06) %lab(08) %lab(10) %lab(12) %lab(14) %lab(16) %lab(18) %lab(20)
...
%lag(08, 06) %lag(10, 08) %lag(12, 10) %lag(14, 12) %lag(16, 14) 
%lag(18, 16) %lag(20, 18)
...
*				NOTE; this is still a different category from first Ei.e. different dummy indicator
...
*wave 16; %ch(16, 0, 14) %ch(16, 1, 12) %ch(16, 2, 10) 
*wave 18; %ch(18, 0, 16) %ch(18, 1, 14) %ch(18, 2, 12) 
*wave 20; %ch(20, 0, 18) %ch(20, 1, 16) %ch(20, 2, 14) 
...
%ch(16, 2, 10) %ch(14, 1, 10) 
%ch(18, 2, 12) %ch(16, 1, 12)
%ch(20, 2, 14) %ch(18, 1, 14)
			   %ch(20, 1, 16)
...
%mend;
%lab(98) %lab(00) %lab(02) %lab(04) %lab(06) %lab(08) %lab(10) %lab(12) %lab(14) %lab(16) %lab(18) %lab(20)
...
%cogn(08) %cogn(10) %cogn(12) %cogn(14) %cogn(16)
%cogn(18) %cogn(20)
...
%macro int(y);
	...
	run;
%mend;
%int(98)%int(00)%int(02)%int(04)%int(06)%int(08)%int(10)%int(12)%int(14)%int(16)%int(18)%int(20)
...
data final; 
	set base (keep = hhid pn hacohort raestrat raehsamp male female NH_white NH_black Hispanic NH_other LTHS HSGED GTHS lths_black lths_hisp); 
run;

%macro wave(y);
	data wave_&y; 
		set base (keep = hhid pn
						 inw_&y iwmo_&y iwyr_&y randchf_&y hrs_wgt_&y
...
%wave(08)%wave(10)%wave(12)%wave(14)%wave(16)%wave(18)%wave(20)
